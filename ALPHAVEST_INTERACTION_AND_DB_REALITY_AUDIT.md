...
| Audit trail | `lib/audit-service.ts`, `lib/typed-workflow-command-bus.ts`, `lib/document-upload-service.ts` | Partial | Partial | Yes in upload/demo workflow | Preview service only | `DB_BACKED_PARTIAL` | Upload and demo workflow write audit rows; `audit-service` itself only previews events. | Audit is not uniformly integrated across all UI actions. |
