...
**Mode:** Final executable implementation handoff. This artefact authorizes Codex implementation only inside explicitly allowed `BP-00` through `BP-11` task boundaries. No implementation, code execution, screen generation, state-screen generation, Prisma migration or new API route creation happened while creating this artefact.  
...
**Artefact status:** `MVP_FIRST_BUILD_IMPLEMENTATION_HANDOFF_APPROVED_WITH_CONSTRAINTS`  
**Implementation gate status:** `IMPLEMENTATION_AUTHORIZED_WITH_CONSTRAINTS`  
**Artefact creation status:** `IMPLEMENTATION_NOT_STARTED_IN_ARTEFACT`

This handoff is the final, constrained execution protocol for Codex. Codex may execute only the allowed task IDs listed here, only against the `full-workflow` target codebase, and only while preserving all Stop Rules, validation duties, P0 positive/negative proof obligations and non-task register exclusions. This Markdown artefact itself does not modify code, run tests, create migrations, create APIs or generate visuals.

| Summary Item | Count / Decision |
| --- | --- |
| Allowed phases | 9 phases, Phase 0 through Phase 8 |
| Allowed task specifications | 62 |
| Allowed source packages | 12 packages: `BP-00` through `BP-11` only |
| Blocked registers | `BLOCKED`, `CONDITIONAL_SUPPORT`, `P1`, `HOLD`, `DO_NOT_CREATE` are non-executable |
| Target repo / branch | `xkyball/alphavest-wealthos-clickdummy` / `full-workflow` |
| Codex may execute? | Yes, but only inside this handoff and only after Phase 0 preflight passes |
| Screen/state/image generation | Not authorized |
| Prisma migrations | Not authorized |
| New API routes by default | Not authorized |
...
| --- | --- | --- | --- |
| Allowed task IDs | Execute only task IDs listed in Section 4. | Invent, remove, rename or silently reorder task IDs. | If any requested task ID cannot be found in Section 4 of this handoff, STOP_AND_REPORT. |
| Allowed packages | Execute `BP-00` through `BP-11` only. | Promote `BP-12`, `BP-P1-*`, `BP-HOLD-*` or `BP-DNC-01`. | Any request to touch non-allowed packages triggers STOP_AND_REPORT. |
...
| UI/visuals | Implement code-level state/feedback where tasks require it. | Generate screens, state-screen images, visual assets or visual replacement. | Any visual generation requirement stops execution. |
| Schema | Align usage to existing full-workflow schema. | Create Prisma migrations, blindly replace schema or create patch-only models. | Any required schema migration triggers STOP_AND_REPORT. |
| APIs | Harden existing APIs where task scope requires it. | Create new API routes by default. | New API route need triggers explicit-source-unlock requirement. |
| Safety | Implement fail-closed RBAC, visibility, evidence, audit and export controls. | Allow anonymous demo, admin bypass, AI Draft leakage, upload-to-release, advisor-as-release or preview-as-approval. | Any safety contradiction triggers STOP_AND_REPORT. |
...
| Must Not Include | Do not use main or older false-gap claims as target truth.; Do not create tasks from main-based missing API/Prisma/tests/UI claims.; Do not create visual, migration or new-API work unless a later final handoff explicitly unlocks it.; Do not treat status chips, buttons, PNGs, route 200s or API 200s as gate proof. |
| Positive Acceptance | Later handoff states full-workflow target and source hierarchy before any task execution.; False-gap register is visible in final task execution preflight.; Task pack and later handoff show NO_SCREEN_GENERATION, NO_SCHEMA_MIGRATION and no-new-API defaults.; Every later task references acceptance and negative P0 proof obligations. |
| Negative P0 Acceptance | Any main-derived absence claim is rejected before task execution.; A main-derived task fails preflight and receives DO_NOT_CREATE_TASK.; Any visual generation/migration/new-API proposal is blocked.; Overclaiming a proof slice blocks phase completion. |
...
| Required Phase Report | Use the mandatory Phase Completion Report template and list completed task IDs, changed files, tests, validation and P0 proof. |
| Stop Rules | Client projection requires new API not authorized by plan.; Existing UI cannot distinguish fail-closed from no data.; API cannot support role-aware payload projection.; Tests cannot inspect client-facing payload. |
...
| UI primitives | `components/ui/drawer.tsx`, `components/ui/modal.tsx`, `components/ui/audit-timeline.tsx`, `components/ui/status-chip.tsx`, `components/ui/workflow-badge.tsx` | SECONDARY_MODIFICATION_TARGET only when a named task requires lifecycle/feedback support | Flow-relevant BP-03..BP-10 tasks | Visible UI is not gate proof; no state-screen/image generation. |
...
| Review monitoring API | `app/api/review-monitoring/route.ts`, `lib/review-monitoring-service.ts` | DEFERRED_P1 / READ_ONLY_BASELINE for exclusion proof only | None by default | No MVP promotion and no automatic advice/rebalance execution. |
...
| Prisma baseline | `prisma/schema.prisma`, `prisma/seed.ts`, migrations folder | READ_ONLY_BASELINE / ALIGN_USAGE_ONLY | All where existing model semantics are needed | NO_SCHEMA_MIGRATION, no blind patch-schema replacement, no patch-only model task. |
...
| P0-NEG-011 | Hold route block | Routes 064–067 and 069–071 do not silently enter MVP task scope. | Held routes remain registered but non-executable/blocked in this handoff. | BP-00, BP-11 | tests/route-smoke.spec.ts; handoff QA | Phase 0/8 completion |
...
| --- | --- | --- | --- | --- | --- |
| pnpm typecheck | Package-specific when referenced; final after Phase 8 | TypeScript contracts compile; does not prove runtime safety. | It does not override Stop Rules, does not prove full MVP safety without mapped positive and negative P0 acceptance, and does not authorize blocked/P1/hold/DNC work. | STOP_AND_REPORT. Capture command, output, suspected cause and impacted task IDs. Do not substitute another script silently. | SCRIPT_PRESENT_IN_SNAPSHOT; if missing at execution, stop and report script mismatch. |
| pnpm lint | Package-specific when referenced; final after Phase 8 | Code style/static lint; does not prove workflow safety. | It does not override Stop Rules, does not prove full MVP safety without mapped positive and negative P0 acceptance, and does not authorize blocked/P1/hold/DNC work. | STOP_AND_REPORT. Capture command, output, suspected cause and impacted task IDs. Do not substitute another script silently. | SCRIPT_PRESENT_IN_SNAPSHOT; if missing at execution, stop and report script mismatch. |
| pnpm db:validate | Package-specific when referenced; final after Phase 8 | Prisma schema validity; does not authorize migrations. | It does not override Stop Rules, does not prove full MVP safety without mapped positive and negative P0 acceptance, and does not authorize blocked/P1/hold/DNC work. | STOP_AND_REPORT. Capture command, output, suspected cause and impacted task IDs. Do not substitute another script silently. | SCRIPT_PRESENT_IN_SNAPSHOT; if missing at execution, stop and report script mismatch. |
| pnpm build | Package-specific when referenced; final after Phase 8 | Next build succeeds; does not prove P0 safety. | It does not override Stop Rules, does not prove full MVP safety without mapped positive and negative P0 acceptance, and does not authorize blocked/P1/hold/DNC work. | STOP_AND_REPORT. Capture command, output, suspected cause and impacted task IDs. Do not substitute another script silently. | SCRIPT_PRESENT_IN_SNAPSHOT; if missing at execution, stop and report script mismatch. |
| pnpm test:playwright | Package-specific when referenced; final after Phase 8 | Browser/test suite execution; scope depends on included specs. | It does not override Stop Rules, does not prove full MVP safety without mapped positive and negative P0 acceptance, and does not authorize blocked/P1/hold/DNC work. | STOP_AND_REPORT. Capture command, output, suspected cause and impacted task IDs. Do not substitute another script silently. | SCRIPT_PRESENT_IN_SNAPSHOT; if missing at execution, stop and report script mismatch. |
| pnpm test:permissions | Package-specific when referenced; final after Phase 8 | Permission-engine proof slices; not full payload matrix unless extended. | It does not override Stop Rules, does not prove full MVP safety without mapped positive and negative P0 acceptance, and does not authorize blocked/P1/hold/DNC work. | STOP_AND_REPORT. Capture command, output, suspected cause and impacted task IDs. Do not substitute another script silently. | SCRIPT_PRESENT_IN_SNAPSHOT; if missing at execution, stop and report script mismatch. |
| pnpm test:workflow-gate | Package-specific when referenced; final after Phase 8 | Workflow gate proof slices; not full release proof unless extended. | It does not override Stop Rules, does not prove full MVP safety without mapped positive and negative P0 acceptance, and does not authorize blocked/P1/hold/DNC work. | STOP_AND_REPORT. Capture command, output, suspected cause and impacted task IDs. Do not substitute another script silently. | SCRIPT_PRESENT_IN_SNAPSHOT; if missing at execution, stop and report script mismatch. |
| pnpm test:workflow-api | Package-specific when referenced; final after Phase 8 | Demo workflow API proof slices; demo success is not production proof. | It does not override Stop Rules, does not prove full MVP safety without mapped positive and negative P0 acceptance, and does not authorize blocked/P1/hold/DNC work. | STOP_AND_REPORT. Capture command, output, suspected cause and impacted task IDs. Do not substitute another script silently. | SCRIPT_PRESENT_IN_SNAPSHOT; if missing at execution, stop and report script mismatch. |
| pnpm test:route-smoke | Package-specific when referenced; final after Phase 8 | Route shell availability; route 200 is not authorization proof. | It does not override Stop Rules, does not prove full MVP safety without mapped positive and negative P0 acceptance, and does not authorize blocked/P1/hold/DNC work. | STOP_AND_REPORT. Capture command, output, suspected cause and impacted task IDs. Do not substitute another script silently. | SCRIPT_PRESENT_IN_SNAPSHOT; if missing at execution, stop and report script mismatch. |
| pnpm test:data-quality | Package-specific when referenced; final after Phase 8 | Data-quality service proof; BP-12 remains conditional. | It does not override Stop Rules, does not prove full MVP safety without mapped positive and negative P0 acceptance, and does not authorize blocked/P1/hold/DNC work. | STOP_AND_REPORT. Capture command, output, suspected cause and impacted task IDs. Do not substitute another script silently. | SCRIPT_PRESENT_IN_SNAPSHOT; if missing at execution, stop and report script mismatch. |
| pnpm test:file-export | Package-specific when referenced; final after Phase 8 | Export/file metadata proof slices; not client acceptance. | It does not override Stop Rules, does not prove full MVP safety without mapped positive and negative P0 acceptance, and does not authorize blocked/P1/hold/DNC work. | STOP_AND_REPORT. Capture command, output, suspected cause and impacted task IDs. Do not substitute another script silently. | SCRIPT_PRESENT_IN_SNAPSHOT; if missing at execution, stop and report script mismatch. |
| pnpm test:phase-d | Package-specific when referenced; final after Phase 8 | Review-monitoring/phase proof slice; P1/hold not promoted. | It does not override Stop Rules, does not prove full MVP safety without mapped positive and negative P0 acceptance, and does not authorize blocked/P1/hold/DNC work. | STOP_AND_REPORT. Capture command, output, suspected cause and impacted task IDs. Do not substitute another script silently. | SCRIPT_PRESENT_IN_SNAPSHOT; if missing at execution, stop and report script mismatch. |
...
- Confirmation that no blocked/P1/hold/DNC scope was touched:
```
...
## 12. Non-Task Register Enforcement

The following registers are non-executable. Codex must prove in every phase report that no blocked, conditional, P1, hold or do-not-create item was touched.

### 12.1 `BLOCKED`

| Register ID | Package / Item | Why Blocked | Required Unlock | Codex Instruction |
| --- | --- | --- | --- | --- |
| REG-BLOCK-001 | BP-BLOCK-01 / stale final-handoff authority | Older task/handoff artefacts do not automatically reflect this First Build task pack. | Use this handoff as controlling source. | Do not execute outside this handoff. |
...
| --- | --- | --- | --- | --- |
| REG-COND-001 | BP-12 — Data quality release-blocking support | Only if explicitly unlocked as first-path release-blocking acceptance proof. | Avoid destabilizing first proof path with ops/data-quality expansion. | Do not implement by default; read-only/exclusion proof only unless later unlock. |

### 12.3 `P1`

| Register ID | Package / Journey | Why P1 | Future Unlock Condition | Codex Instruction |
| --- | --- | --- | --- | --- |
| REG-P1-001 | BP-P1-01 / MJ-008 — Review monitoring internal review not automatic advice | Useful review rhythm after MVP, not first product proof. | Core MVP release/visibility/advice gates accepted. | Do not implement in first build; no automatic advice. |
| REG-P1-002 | BP-P1-02 / MJ-009 — Client mobile / communication workflow | Client/service layer after core release/visibility safety. | Client-safe projection and advice gates accepted. | Do not implement in first build. |
| REG-P1-003 | BP-P1-03 / MJ-011 — External advisor scoped access | Guest access compounds object-scope risk. | BP-01 and BP-03 accepted plus explicit object-scope need. | Do not implement unscoped guest/external advisor access. |

### 12.4 `HOLD`

| Register ID | Package / Journey / Routes | Why Hold | Required Unlock | Codex Instruction |
| --- | --- | --- | --- | --- |
| REG-HOLD-001 | BP-HOLD-01 / MJ-004 / routes 070–071 | Committee review/dissent depends on held routes and unresolved governance depth. | Dedicated committee unlock artefact with route, visual, safety, action and test decisions. | Do not modify or promote routes 070–071. |
| REG-HOLD-002 | BP-HOLD-02 / MJ-007 / routes 064–067 | KYC/SoW/Suitability/IPS depend on held routes and regulated advice semantics. | Dedicated KYC/suitability unlock artefact with legal/safety scope. | Do not modify or promote routes 064–067. |
| REG-HOLD-003 | Route 069 — Rebalance monitoring | Held/P1-adjacent and not first proof. | Explicit review-monitoring/no-auto-advice unlock. | Do not promote route 069 into first MVP build. |

### 12.5 `DO_NOT_CREATE`

| Register ID | Item | Violated Rule | Codex Instruction |
| --- | --- | --- | --- |
| REG-DNC-001 | Autonomous advice | No autonomous advice / human-backed MVP | Do not create autonomous recommendation execution. |
| REG-DNC-002 | Client-visible AI Draft | AI_DRAFT_INTERNAL_ONLY | Do not expose AI Draft/internal rationale/compliance notes to client/API/export. |
| REG-DNC-003 | Admin bypass | ADMIN_NON_BYPASS_REQUIRED | Do not create superadmin release/sufficiency/export/visibility override. |
| REG-DNC-004 | Upload-to-release shortcut | UPLOAD_ONLY_NOT_SUFFICIENCY | Do not unlock release/export/client visibility from upload. |
| REG-DNC-005 | Blind patch-schema replacement | DO_NOT_REPLACE_FULL_SCHEMA | Do not replace full-workflow Prisma schema with patch schema. |
| REG-DNC-006 | Screen generation | NO_SCREEN_GENERATION | Do not generate or replace screens. |
| REG-DNC-007 | State-screen image generation | NO_STATE_SCREEN_GENERATION | Do not generate state-screen images. |
| REG-DNC-008 | Visual replacement | NO_VISUAL_REPLACEMENT | Do not replace visual assets. |
| REG-DNC-009 | main-derived target task | FULL_WORKFLOW_ONLY | Do not create tasks from `main` absence claims. |
| REG-DNC-010 | Unscoped external advisor access | OBJECT_SCOPE_REQUIRED | Do not create unscoped guest/external advisor access. |
| REG-DNC-011 | Direct P1-to-MVP promotion | P1_DEFERRED_NO_EXECUTION | Do not promote BP-P1-* into MVP first build. |
| REG-DNC-012 | Direct hold-to-MVP promotion | HOLD_NO_EXECUTION | Do not promote BP-HOLD-* or held routes into first build. |
| REG-DNC-013 | New API route by default | NO_NEW_API_BY_DEFAULT | Do not create new API routes unless a later handoff explicitly authorizes one. |
| REG-DNC-014 | Prisma migration from this handoff | NO_SCHEMA_MIGRATION | Do not create migrations from this handoff. |
...
| Only BP-00..BP-11 executable | PASS required | PASS | Covered in this handoff; Codex must reconfirm during Phase 0/phase reports. |
...
| No screen/image/state generation | PASS required | PASS | Covered in this handoff; Codex must reconfirm during Phase 0/phase reports. |
| No schema migration | PASS required | PASS | Covered in this handoff; Codex must reconfirm during Phase 0/phase reports. |
| No new API by default | PASS required | PASS | Covered in this handoff; Codex must reconfirm during Phase 0/phase reports. |
| `main` blocked | PASS required | PASS | Covered in this handoff; Codex must reconfirm during Phase 0/phase reports. |
| BP-12 not default | PASS required | PASS | Covered in this handoff; Codex must reconfirm during Phase 0/phase reports. |
| P1 deferred | PASS required | PASS | Covered in this handoff; Codex must reconfirm during Phase 0/phase reports. |
| HOLD blocked | PASS required | PASS | Covered in this handoff; Codex must reconfirm during Phase 0/phase reports. |
| DNC not taskified | PASS required | PASS | Covered in this handoff; Codex must reconfirm during Phase 0/phase reports. |
...
Execute only ALLOWED_TASK entries listed in this handoff.
Use only the full-workflow target codebase.
Do not use main as target truth.
Do not implement BLOCKED, CONDITIONAL_SUPPORT, P1, HOLD or DO_NOT_CREATE items.
Do not generate screens, state-screen images or visual assets.
Do not create Prisma migrations.
Do not create new API routes unless this handoff explicitly authorizes one.
...
| Only BP-00 through BP-11 authorized | PASS | Explicitly encoded in Source Lock, Authorization Boundary, task inventory, P0 matrix, Stop Rules, command plan or non-task registers. |
| 9-phase sequence preserved | PASS | Explicitly encoded in Source Lock, Authorization Boundary, task inventory, P0 matrix, Stop Rules, command plan or non-task registers. |
| 62 task specifications preserved | PASS | Explicitly encoded in Source Lock, Authorization Boundary, task inventory, P0 matrix, Stop Rules, command plan or non-task registers. |
...
| BP-12 conditional only | PASS | Explicitly encoded in Source Lock, Authorization Boundary, task inventory, P0 matrix, Stop Rules, command plan or non-task registers. |
| P1 deferred | PASS | Explicitly encoded in Source Lock, Authorization Boundary, task inventory, P0 matrix, Stop Rules, command plan or non-task registers. |
| HOLD blocked | PASS | Explicitly encoded in Source Lock, Authorization Boundary, task inventory, P0 matrix, Stop Rules, command plan or non-task registers. |
| DNC forbidden | PASS | Explicitly encoded in Source Lock, Authorization Boundary, task inventory, P0 matrix, Stop Rules, command plan or non-task registers. |
...
| No screen/image/state generation authorized | PASS | Explicitly encoded in Source Lock, Authorization Boundary, task inventory, P0 matrix, Stop Rules, command plan or non-task registers. |
| No schema migration authorized | PASS | Explicitly encoded in Source Lock, Authorization Boundary, task inventory, P0 matrix, Stop Rules, command plan or non-task registers. |
| No new API route by default | PASS | Explicitly encoded in Source Lock, Authorization Boundary, task inventory, P0 matrix, Stop Rules, command plan or non-task registers. |
...
| Branch Build | ENGINE_v2 + ENGINE_v3 | Allowed execution branch and non-executable registers separated. |
| Implementation Handoff Discipline | ENGINE_v2-B | Handoff made executable for Codex only inside explicit task/phase/file/test boundaries. |
...
| BP-11 tasks | BP-11 | All MVP and flow-relevant support routes | All accepted task surfaces | P0 proof | Positive/negative P0 tests cover the accepted first-build path. | No-overclaim feedback and fail-closed proof. | All P0 gates | P1/HOLD/DNC excluded. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| BP-00 | BP-00 | All existing APIs as reference | Existing only | Do not overclaim API presence. | N/A | N/A | API 200 is not safety proof. | Any ambiguity stops execution. | P0-NEG-012 | NO_NEW_API_BY_DEFAULT |
...
| Register only | P1/HOLD reference | `/api/review-monitoring` | Existing but deferred | Read-only/exclusion proof unless future unlock. | N/A in MVP first build. | N/A in MVP first build. | No automatic advice execution. | P1/default deferred. | review-monitoring service remains proof slice only. | NO |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| All allowed tasks | BP-00..BP-11 | Full-workflow schema baseline: User, ClientTenant, Role, Permission, UserRole, Document, EvidenceRecord, Recommendation, Approval, ComplianceReview, Decision, AuditEvent, ExportRequest and related fields. | Align code usage to existing full-workflow model semantics. | 42 models / 22 enums; schema is baseline. | Only usage alignment and field mapping allowed unless future handoff explicitly unlocks schema work. | RBAC, tenant isolation, evidence, advice boundary, audit, export. | P0 positive/negative gates. | NO_SCHEMA_MIGRATION; ALIGN_USAGE_ONLY |
| BP-01/BP-02/BP-03 | Identity/Governance/RBAC | User, UserProfile, ClientTenant, Role, Permission, UserRole, RolePermission, AccessRequest, SecondConfirmation, PolicyDefinition. | Current actor/tenant/role/object scope and admin non-bypass. | Use existing models and policy fields. | Do not create patch-only auth/RBAC models. | RBAC and admin non-bypass. | P0-NEG-003, P0-NEG-010, P0-NEG-012. | NO_SCHEMA_MIGRATION |
| BP-04/BP-07/BP-09 | Evidence/Compliance/Audit | Document, DocumentVersion, DocumentExtraction, DocumentReview, DocumentLink, EvidenceRecord, EvidenceItem, ComplianceReview, AuditEvent. | Evidence lifecycle, compliance preconditions and audit persistence. | Use existing document/evidence/compliance/audit models. | If exact field is missing, stop and report before inventing schema. | Upload not sufficiency, evidence sufficiency, audit. | P0-NEG-004, P0-NEG-005, P0-NEG-009. | NO_SCHEMA_MIGRATION |
| BP-05/BP-06/BP-08/BP-10 | Draft/Advisor/Client/Export | Trigger, Recommendation, RecommendationOption, Approval, Decision, ExportRequest, EvidenceRecord, AuditEvent. | Internal draft, advisor approval, client-safe projection and export redaction. | Use existing recommendation/approval/decision/export models and service-derived visibility. | No explicit AiDraft patch model unless future unlock; represent according to existing schema/service semantics. | AI internal-only, no unapproved advice, export redaction. | P0-NEG-001, P0-NEG-002, P0-NEG-006, P0-NEG-007. | NO_SCHEMA_MIGRATION |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| BP-00 task group | BP-00 | See package task rows and current `tests/*.spec.ts` baseline. | Existing tests are proof slices for route/API/service behaviour. | It does not prove full P0 safety unless package-specific positive and negative criteria are added/updated and pass. | Package-specific positive acceptance from allowed tasks. | Any main-derived absence claim is rejected before task execution.; A main-derived task fails preflight and receives DO_NOT_CREATE_TASK.; Any visual generation/migration/new-API proposal is blocked.; Overclaiming a proof slice blocks phase completion. | P0-NEG-012; P0_HOLD_ROUTE_BLOCK_GATE; All P0 gates | P0_POSITIVE_REQUIRED; P0_NEGATIVE_REQUIRED |
