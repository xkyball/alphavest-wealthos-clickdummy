...
The app still intentionally avoids real authentication, real client data, final financial/legal/tax advice and production identity-provider authorization. Document intake now persists binary uploads through a storage adapter, can run against local filesystem storage or S3-compatible MinIO, and generates persisted preview/thumbnail derivatives for product-native document review surfaces. Compliance release remains the guard for client visibility, and no unapproved advice should reach the client.
