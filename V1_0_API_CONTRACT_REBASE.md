...
| `deleted generic workflow route` | `POST` | MVP/support action transport | JSON action body parsed by `parseTypedWorkflowRequestBody` | Action result or fail-closed error | Permission, audit, evidence, export and workflow gates are action-family specific | `HARDENED`; uses `failClosedJson` |
