...
| `wrong_role` | Wrong role is denied for protected action and receives no authority expansion from route visibility. | `B-006`, `B-010`, `G-005`, `J-009` | `tests/av27-safety-foundation.spec.ts`, `tests/recommendation-review-workflow-api.spec.ts` |
...
| `no_audit` | Critical action cannot complete silently when durable audit is unavailable. | `B-012`, `G-006`, `I-007` | `tests/audit-fail-closed.spec.ts`, `tests/recommendation-review-workflow-api.spec.ts`, `tests/data-quality-service.spec.ts` |
...
| `advisor_not_release` | Advisor approval moves to compliance-pending only and cannot create client release. | `F-005`, `G-006`, `G-009`, `I-004` | `tests/workflow-gate.spec.ts`, `tests/recommendation-review-workflow-api.spec.ts` |
