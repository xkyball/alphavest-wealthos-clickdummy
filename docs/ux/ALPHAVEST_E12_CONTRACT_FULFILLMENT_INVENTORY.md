...
| E09-CAPTURE-QA | capture QA / release capture evidence | `docs/ux/ALPHAVEST_E09_CAPTURE_QA_SPEC.md` | `scripts/capture-qa-contract.ts`, `scripts/visual-qa-contract.ts`, `tests/capture-qa-contract.spec.ts`, `tests/e09-capture-release-policy.spec.ts`, `visual:capture-qa:release` | partial | New release captures must use `CAPTURE_QA_FAIL_ON_WARNINGS=1`; old bundles are historical only. |
...
| Capture release QA | `pnpm visual:capture-qa:release` | Uses `CAPTURE_QA_FAIL_ON_WARNINGS=1`; valid E09 hardening anchor. |
