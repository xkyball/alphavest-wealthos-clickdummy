...
- Entries for `operational_visual_audit` must include `visual:audit-operational` and `tests/operational-visual-audit.spec.ts`.
...
- E09 operational screenshot audit release proof.
...
| `E12-GATE-OPERATIONAL-SCREENSHOT-AUDIT` | failure for release mode | package scripts and operational screenshot audit test | `Release screenshot proof must run pnpm visual:audit-operational and must not reference retired capture-QA scripts.` |
...
### Operational Screenshot Audit Release Proof

Operational screenshot audit is a release-proof contract, not a screenshot convention.

Rules:

- Every UI screenshot proof must be paired with `pnpm visual:audit-operational`.
- `release:contract-check` must include `pnpm visual:audit-operational`.
- Retired scripts `scripts/capture-qa-contract.ts` and `scripts/strict-visual-capture.ts` must not re-enter package scripts, source imports or release gates.
- Capture generators may still produce artifacts, but they do not own screenshot acceptance.
...
- Release report points to a screenshot without operational audit: fail.
- Package script drops `pnpm visual:audit-operational`: fail.
- Package script reintroduces `visual:capture-qa`, `visual:capture-qa:release` or `visual:strict`: fail.
...
| E09 operational screenshot audit | `playwright test tests/operational-visual-audit.spec.ts tests/e09-capture-release-policy.spec.ts` |
...
| Release definition separates source guard, smoke, contract fulfillment, operational screenshot audit and focused tests. | PASS |
...
| Release definition | Minimum release bundle now includes `pnpm guard:source`, `pnpm test:route-smoke`, `pnpm test:contract-fulfillment`, and `pnpm visual:audit-operational` before screenshots can be used as proof. |
