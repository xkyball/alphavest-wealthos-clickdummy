...
| Workflow services | `lib/internal-workflow-demo-data.ts`, `lib/demo-workflow-mutation.ts`, `lib/workflow-gate.ts`, `lib/visibility-engine.ts`, `lib/audit-service.ts` exist. | Reuse/harden existing workflow/gate/visibility/audit concepts; do not bypass them in UI state. |
...
| Services/gates | `lib/internal-workflow-demo-data.ts`, `lib/demo-workflow-mutation.ts`, `lib/demo-workflow-validation.ts`, `lib/workflow-gate.ts`, `lib/permission-engine.ts`, `lib/visibility-engine.ts`, `lib/audit-service.ts` | Confirm analyst action preconditions, role checks, AI draft visibility and audit path. |
...
| `lib/demo-workflow-mutation.ts` | Workflow mutation baseline; must not bypass gates. |
