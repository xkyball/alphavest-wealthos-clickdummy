...
| API/service path | `app/api/demo-workflow/route.ts`, `lib/demo-workflow-mutation.ts`, `lib/workflow-gate.ts`, `lib/permission-engine.ts`, `lib/audit-service.ts`, `lib/visibility-engine.ts` exist in snapshot. | Use/harden existing workflow action path. Do not add API unless WP-00 proves no safe existing route. |
...
| `lib/demo-workflow-mutation.ts` | Workflow mutation implementation. |
