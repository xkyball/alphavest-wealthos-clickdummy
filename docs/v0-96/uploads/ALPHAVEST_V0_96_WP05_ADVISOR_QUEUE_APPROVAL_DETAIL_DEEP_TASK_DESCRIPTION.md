...
| API/service path | `deleted generic workflow route`, `lib/typed-workflow-command-bus.ts`, `lib/workflow-gate.ts`, `lib/permission-engine.ts`, `lib/audit-service.ts`, `lib/visibility-engine.ts` exist in snapshot. | Use/harden existing workflow action path. Do not add API unless WP-00 proves no safe existing route. |
| Tests | `workflow-gate.spec.ts`, `recommendation-review-workflow-api.spec.ts`, `permission-engine.spec.ts`, `p0-acceptance.spec.ts`, `true-ux-cta-state.spec.ts`, `true-ux-p0-safety.spec.ts`, `scf-p04-p06-flow-ui.spec.ts` exist in snapshot. | Extend existing tests where possible; add named tests only if current coverage lacks required negative/positive proof. |
...
|---|---|
| `deleted generic workflow route` | Existing action transport for advisor approve/reject/request changes if current code uses it. |
...
| `lib/recommendation-review-workflow-validation.ts` | Request/action validation. |
...
9. Avoid creating a second advisor workflow path if existing typed workflow can be hardened.
