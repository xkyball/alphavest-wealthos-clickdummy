...
| Audit services | `lib/audit-service.ts`, `lib/permission-engine.ts`, `lib/demo-workflow-mutation.ts`, `lib/workflow-gate.ts`, `lib/export-service.ts`, `lib/evidence-service.ts` | Verify write path, failure handling, service usage |
...
| `lib/demo-workflow-mutation.ts` | Demo action audit counts must not be overclaimed as production proof. |
