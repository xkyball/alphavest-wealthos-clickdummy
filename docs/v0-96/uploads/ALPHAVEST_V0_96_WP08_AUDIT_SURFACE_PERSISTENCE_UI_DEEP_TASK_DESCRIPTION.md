...
| Existing tests | `tests/audit-fail-closed.spec.ts`, `tests/phase6-audit-persistence.spec.ts`, `tests/permission-engine.spec.ts`, `tests/governance-non-bypass.spec.ts`, `tests/document-upload-api.spec.ts`, `tests/recommendation-review-workflow-api.spec.ts` | Reuse/harden existing specs; do not duplicate names blindly |
...
| Are typed workflow audit rows production-like or demo-only? | Label demo audit as proof slice only; do not overclaim production persistence. |
