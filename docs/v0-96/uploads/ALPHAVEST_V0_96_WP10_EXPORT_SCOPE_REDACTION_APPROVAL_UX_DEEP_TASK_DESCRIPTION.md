...
| Export API | `app/api/export-workflow/route.ts`; existing `app/api/demo-workflow/route.ts` export actions if still used | Confirm request/response contracts, actor checks, validation, safe errors, audit behavior. |
...
| `WP10-T10-EXPORT-API-HARDENING` | Harden export API request/response/error contract. | API route presence is not safety proof. | `app/api/export-workflow/route.ts`, `app/api/demo-workflow/route.ts` if export actions remain there | Validate request action, export ID, actor, tenant, scope, redaction status, approval state. Return safe error envelope without internal payload. Do not advance state on error. | API supports truthful UI states and fail-closed errors. | `phase8-export-workflow-api.spec.ts`, `p0-api-contract.spec.ts`. | Indirect: UI receives safe state/errors. | Stop if API returns raw internals or changes state after denied/error. |
...
| Legacy/demo workflow export actions | `app/api/demo-workflow/route.ts` if still used after rebase |
