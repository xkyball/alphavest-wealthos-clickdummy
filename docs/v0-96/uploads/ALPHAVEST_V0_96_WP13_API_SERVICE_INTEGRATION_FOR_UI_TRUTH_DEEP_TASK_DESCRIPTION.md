...
- Relevant API/service surfaces already exist for dummy auth, admin tenants, documents, document review, audit events, export workflow, demo workflow, dashboard metrics, global search, entities, family members, profile, ops SLA and review monitoring.
...
| `app/api/demo-workflow/route.ts` | Existing demo action transport | Reuse/harden; do not treat demo success as production persistence proof |
...
| `lib/demo-workflow-validation.ts` | Request validation for demo workflow actions |
| `lib/demo-workflow-mutation.ts` | Mutation result/audit/state transition proof limits |
...
The current application contains many UI surfaces, status labels, actions, demo workflows and tests, but WP-13 must prevent a specific failure mode:
