...
| Demo workflow J01 canonical typed boundary | `j01.requestData` preserves `clientVisible=false` on the canonical typed boundary (legacy compatibility bridge) and keeps internal review flowing before release. | Useful canonical typed boundary continuity and advisor-not-release proof candidate. |
