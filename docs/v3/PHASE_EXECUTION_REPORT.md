...
- Result: `DUMMY_DB_AUTH_MFA_INVITE_IMPLEMENTED_WITH_MANDATORY_DEMO_LOGIN_GUARD`.
...
- Added `proxy.ts` as the app-route auth boundary: unauthenticated non-auth pages redirect to `/login?returnTo=...`; authenticated visits to `/login` and `/` redirect to `/portal`.
- Added `lib/dummy-auth-session.ts` for the shared demo session cookie name, max age and token-shape guard.
...
- Set the HTTP-only demo session cookie after successful MFA verification or invite acceptance.
...
- Route guard changes: `proxy.ts` enforces the demo auth session cookie before app pages render while leaving `/login`, `/mfa`, `/onboarding/*`, API routes and static assets available.
...
- `PLAYWRIGHT_PORT=3217 pnpm test:dummy-auth` - passed, 5 tests on a fresh server, including mandatory-login route guard proof.
...
- Mandatory login acceptance: PASS for `/portal` redirecting to `/login?returnTo=%2Fportal` without a demo session cookie and rendering only after a valid `av-session-*` demo cookie exists.
...
- Known gap: this creates demo-grade MFA and a demo cookie guard with a deterministic code (`123456`); it does not implement device binding, TOTP enrollment, recovery codes, signed production sessions or real provider federation.
...
## UX-HUB Workstream Execution Addendum
...
Executed `UX-HUB` as the second UX refactoring workstream. This pass preserved the Route Policy Matrix, P0 obligations, route workset boundaries and no-screen-generation rule while replacing hub-like route surfaces with a dedicated migrated hub component.

### Completed Workstream / Tasks

- `UX-HUB`: Added a dedicated `UxHubPanel` component and `uxHubGuidanceForPageId` policy data for hub routes.
- Migrated existing hub routes into the dedicated hub pattern instead of treating hubs as a generic guidance overlay.
- Covered page IDs `013`, `015`, `019`, `020`, `024`, `031`, `034`, `043` and `054` with route-specific priority cards, safe next-work queues and non-final primary CTAs.
- Preserved client visibility and approval safety by keeping hub primary CTAs away from confirm, download, share, release and acceptance actions.
- Added route-smoke regression coverage proving the migrated hub surface appears on the intended existing routes.
...
- `pnpm typecheck` - passed.
- `pnpm exec playwright test tests/route-smoke.spec.ts -g "UX-HUB"` - passed, 9 tests.
- `pnpm lint` - passed.
- `pnpm test:permissions` - passed, 8 tests.
- `PLAYWRIGHT_PORT=3031 pnpm test:route-smoke` - passed, 99 tests.
- Screenshot capture on `http://127.0.0.1:3337` - passed for 6 representative hub routes.

### Positive Acceptance

- Hub pages now use a dedicated route-policy-backed component with page-specific status, priorities and next-work queues.
- Existing routes were migrated into the hub component without adding new routes or screens.
- Hub primary CTAs are safe continuation links and do not perform final release, share, download, confirm or acceptance actions.
- Browser screenshots exist for admin setup, portal, mobile, workbench, decisions and export hub surfaces.

### Negative Acceptance

- No route was reclassified in the Route Policy Matrix.
- No new product route, API route, Prisma migration, state-screen/image generation, production auth claim, autonomous advice path or client-visible AI draft was introduced.
- Advisor approval, hub presence and route visibility do not bypass evidence sufficiency, compliance release or export controls.

### Exit Gate Decision

`UX_HUB_IMPLEMENTED_WITH_MIGRATED_ROUTE_SCREENSHOT_PROOF`
