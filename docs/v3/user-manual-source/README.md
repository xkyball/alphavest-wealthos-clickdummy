...
Current AlphaVest is a route-complete, data-model-rich demo workflow prototype with partial executable workflow behavior. J01 signal/advisor actions remain a legacy compatibility bridge while canonical typed journey governance is migrated. Most other workflows are navigable/static or route-state demos and must not be described as fully persisted or governed in a finished manual until the implementation gaps are closed.
