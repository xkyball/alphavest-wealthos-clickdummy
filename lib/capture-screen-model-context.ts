...
const groupTemplates: Record<NavigationGroupKey, ContextTemplate> = {
  ...
  client_workspace: {
    capability: {
      ...
      guardEvidence: ["Tenant/object permission checks; upload/review safety branches."],
      proofPosture: "Strong candidate for profile, family, entity, document and evidence flows; full DB/browser suites still decide completion.",
      serviceEvidence: ["lib/dbtf-form-service.ts", "lib/dbtf-table-service.ts", "lib/document-upload-service.ts", "lib/evidence-review-service.ts"],
    ...
    warnings: ["Schema breadth does not prove every field is editable."],
  },
  wealth_actions: {
    capability: {
