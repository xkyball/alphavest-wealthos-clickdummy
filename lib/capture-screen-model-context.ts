...
const groupTemplates: Record<NavigationGroupKey, ContextTemplate> = {
  access: {
    capability: {
      ...
      serviceEvidence: ["lib/auth/local-auth-provider-service.ts", "lib/auth/current-user.ts"],
  ...
  tenant_setup: {
    capability: {
      ...
      serviceEvidence: ["lib/admin-tenant-governance-service.ts", "lib/auth/local-auth-provider-service.ts", "lib/tenant-governance-command-client.ts", "lib/tenant-governance-workflow-actions.ts"],
      status: "API_BACKED_PARTIAL",
    },
    modelFamilies: ["tenant-user-rbac"],
