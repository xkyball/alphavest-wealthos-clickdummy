...
export type P44Phase2TicketStatus = "complete_direct" | "complete_boundary" | "complete_certification";

export type P44Phase2Ticket = {
  acceptance: string;
  description: string;
  id: string;
  negative: string;
  proof: string[];
  status: P44Phase2TicketStatus;
};

export class P44Phase2ValidationError extends Error {
  constructor(readonly issues: string[]) {
    super("Invalid P44 Phase 2 request.");
  }
}

export class P44Phase2PermissionError extends Error {
...
async function writeP44Audit(
  ...
  return prisma.auditEvent.create({
    data: {
      ...
      metadataJson: {
        p44Phase: "PH2",
        p44Ticket: input.eventType.split(".").slice(0, 3).join("."),
...
async function assertActor(
  ...
  const audit = await writeP44Audit(prisma, {
    actorRoleKey,
    clientTenantId: input.clientTenantId,
    eventType: `${input.eventType}.denied`,
    reason: `${actorRoleKey} cannot perform this P44 Phase 2 admin foundation action.`,
    result: AuditResult.DENIED,
    targetId: input.targetId,
    targetType: input.targetType,
  }).catch(() => undefined);

  throw new P44Phase2PermissionError("P44_PHASE2_ACTOR_DENIED", `${actorRoleKey} cannot perform this action.`, audit?.id);
}

export async function createP44ClientTenant(
  prisma: PrismaClient,
  input: {
  ...
  const targetId = stableId(`p44-phase2:client-tenant:${displayName.toLowerCase()}`);

  await assertActor(prisma, actorRole, adminActionRoles, {
    eventType: "p44.phase2.tenant_create",
    targetId,
  ...
  if (issues.length > 0) {
    throw new P44Phase2ValidationError(issues);
  }
  ...
  if (duplicate) {
    const audit = await writeP44Audit(prisma, {
      ...
      eventType: "p44.phase2.tenant_create.duplicate_denied",
    ...
    throw new P44Phase2PermissionError("P44_PHASE2_DUPLICATE_TENANT", "Duplicate client tenant rejected.", audit.id);
  }
  ...
  const tenant = await prisma.$transaction(async (tx) => {
    ...
    await writeP44Audit(tx, {
      ...
      eventType: "p44.phase2.tenant_create.success",
      metadataJson: { jurisdiction, relationshipTier },
      nextState: TenantStatus.DRAFT,
      previousState: "NONE",
      reason: "Client tenant draft created through P44 Phase 2 UI/API/DB path.",
...
export async function updateP44PlatformSetting(
  prisma: PrismaClient,
  input: {
    actorRoleKey?: unknown;
  ...
  const targetId = stableId(`p44-phase2:platform-setting:${settingKey || "invalid"}`);

  await assertActor(prisma, actorRole, adminActionRoles, {
    eventType: "p44.phase2.platform_setting",
    targetId,
    targetType: ObjectType.PLATFORM,
  });

  if (!settingKey || !Number.isInteger(retentionYears) || retentionYears < 1 || retentionYears > 25) {
    throw new P44Phase2ValidationError(["valid_setting_key_and_retention_required"]);
  }

  const audit = await writeP44Audit(prisma, {
    actorRoleKey: actorRole,
    eventType: "p44.phase2.platform_setting.success",
...
export async function updateP44SecurityConfiguration(
  prisma: PrismaClient,
  input: {
    actorRoleKey?: unknown;
    mfaRequired?: unknown;
  ...
  const targetId = stableId("p44-phase2:security-configuration");

  await assertActor(prisma, actorRole, securityActionRoles, {
    eventType: "p44.phase2.security_configuration",
    targetId,
  ...
  if (!mfaRequired || !Number.isInteger(sessionMinutes) || sessionMinutes < 5 || sessionMinutes > 120) {
    const audit = await writeP44Audit(prisma, {
      actorRoleKey: actorRole,
      eventType: "p44.phase2.security_configuration.blocked",
    ...
    throw new P44Phase2PermissionError("P44_PHASE2_SECURITY_FAIL_CLOSED", "Security configuration failed closed.", audit.id);
  }

  const audit = await writeP44Audit(prisma, {
    actorRoleKey: actorRole,
    eventType: "p44.phase2.security_configuration.success",
    metadataJson: { mfaRequired, sessionMinutes },
...
export async function createP44PolicyVersion(
  prisma: PrismaClient,
  input: {
    actorRoleKey?: unknown;
  ...
  const targetId = stableId(`p44-phase2:policy:${tenant?.id ?? "platform"}:${policyKey}:${version}`);

  await assertActor(prisma, actorRole, policyActionRoles, {
    clientTenantId: tenant?.id,
    eventType: "p44.phase2.policy_version",
    targetId,
    targetType: ObjectType.PERMISSION,
  });

  if (!policyKey || !version || !["draft", "active", "retired"].includes(status)) {
    throw new P44Phase2ValidationError(["valid_policy_key_version_status_required"]);
  }
  ...
  const policy = existing
    ...
    : await prisma.policyDefinition.create({
        data: {
          category: "p44_phase2_policy",
          clientTenantId: tenant?.id,
          createdByUserId: stableId(`p44-phase2:policy-actor:${actorRole}`),
          effectiveFrom: status === "active" ? new Date() : null,
          id: targetId,
          name: `P44 ${policyKey}`,
          platformTenantId: demoPlatformTenantId,
          policyKey,
          rulesJson: { p44Phase: "PH2", source: "p44_phase2_admin_foundation" },
          status,
          version,
        },
      });
  ...
  const audit = await writeP44Audit(prisma, {
    actorRoleKey: actorRole,
    clientTenantId: tenant?.id,
    eventType: "p44.phase2.policy_version.success",
...
export async function requireP44EffectivePolicy(prisma: PrismaClient, input: { clientTenantSlug?: unknown; policyKey?: unknown }) {
  const policyKey = text(input.policyKey, 120);
  const parsedTenantSlug = tenantSlug(input.clientTenantSlug);
  const tenant = parsedTenantSlug ? tenantForSlug(parsedTenantSlug) : null;
  ...
  if (!policy) {
    throw new P44Phase2PermissionError("P44_PHASE2_EFFECTIVE_POLICY_REQUIRED", "Effective policy is required.");
  }

  return policy;
}

export async function assignP44TeamMember(
  prisma: PrismaClient,
  input: {
    actorRoleKey?: unknown;
    email?: unknown;
  ...
  if (!assigneeEmail || !parsedRoleKey || !parsedTenantSlug) {
    throw new P44Phase2ValidationError(["valid_email_role_tenant_required"]);
  }

  const tenant = tenantForSlug(parsedTenantSlug);
  await assertActor(prisma, actorRole, adminActionRoles, {
    clientTenantId: tenant.id,
    eventType: "p44.phase2.team_assignment",
    targetId: tenant.id,
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const assignment = await tx.userRole.upsert({
      create: {
        assignedByUserId: stableId(`p44-phase2:assigner:${actorRole}`),
        clientTenantId: tenant.id,
        id: stableId(`p44-phase2:user-role:${tenant.slug}:${assigneeEmail}:${parsedRoleKey}`),
        roleId: role.id,
        status: "active",
        userId: user.id,
        validFrom: new Date(),
      ...
      where: { id: stableId(`p44-phase2:user-role:${tenant.slug}:${assigneeEmail}:${parsedRoleKey}`) },
    });

    const audit = await writeP44Audit(tx, {
      actorRoleKey: actorRole,
      actorUserId: stableId(`p44-phase2:assigner:${actorRole}`),
      clientTenantId: tenant.id,
      eventType: "p44.phase2.team_assignment.success",
...
export const p44Phase2Tickets: P44Phase2Ticket[] = [
  {
    ...
    proof: ["createP44ClientTenant", "app/api/admin-tenants POST create_tenant", "getAdminTenantSnapshot"],
  ...
  },
  {
    ...
    proof: ["saveDbtfClientProfile", "updateDbtfFamilyMember", "tests/av27-client-context-closure.spec.ts"],
  ...
  },
  {
    ...
    proof: ["saveDbtfEntityWizard", "listDbtfEntities", "tests/av27-client-context-closure.spec.ts"],
    status: "complete_boundary",
  },
  {
  ...
  },
  {
    ...
    proof: ["tests/av27-client-context-closure.spec.ts"],
    status: "complete_boundary",
  },
  {
    ...
    proof: ["updateP44PlatformSetting"],
    status: "complete_direct",
  },
  {
    ...
    proof: ["updateP44SecurityConfiguration"],
    status: "complete_direct",
  },
  {
    ...
    proof: ["createP44PolicyVersion", "requireP44EffectivePolicy"],
    status: "complete_direct",
  },
  {
    ...
    proof: ["tests/p44-phase2-certification.spec.ts"],
  ...
  },
  {
    ...
    proof: ["writeP44Audit", "tests/p44-phase2-certification.spec.ts"],
    status: "complete_certification",
  },
  {
    acceptance: "A and L processes have selected L2/L3 proof with blockers documented.",
    description: "Phase 2 exit certification.",
    id: "P44-2-T12-IMPL",
    negative: "No downstream process assumes unscoped context.",
    proof: ["p44Phase2Tickets", "tests/p44-phase2-certification.spec.ts"],
    status: "complete_certification",
  },
];

export function getP44Phase2Certification() {
  return {
    complete: p44Phase2Tickets.every((ticket) => ticket.status.startsWith("complete")),
    noClientRelease: true,
    phase: "PH2",
    tickets: p44Phase2Tickets,
  };
}
