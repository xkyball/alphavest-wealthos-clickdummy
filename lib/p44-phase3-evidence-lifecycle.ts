...
type P44Prisma = PrismaClient | Prisma.TransactionClient;

export type P44EvidenceWorkflowSource = "advisor" | "compliance" | "internal";

export type P44EvidenceFeedbackState =
  | "loading"
  | "validation_failed"
  | "blocked"
  | "success"
...
export type P44EvidenceLifecycleState =
  | "request_open"
  | "upload_pending"
...
export class P44Phase3ValidationError extends Error {
  constructor(public readonly issues: string[]) {
    super("Invalid P44 Phase 3 evidence lifecycle request.");
  }
}

export class P44Phase3PermissionError extends Error {
  constructor(public readonly reasonCode: string) {
    super("P44 Phase 3 evidence lifecycle action is not permitted.");
  }
}
...
function requireReason(reason: string) {
  if (!reason.trim()) {
    throw new P44Phase3ValidationError(["reason_required"]);
  }
}

function assertRequestRole(roleKey: DemoRoleKey) {
  if (!requestRoleAllowlist.has(roleKey)) {
    throw new P44Phase3PermissionError("evidence_request_role_denied");
  }
}

function assertComplianceRole(roleKey: DemoRoleKey) {
  if (!complianceOnlyRoles.has(roleKey)) {
    throw new P44Phase3PermissionError("compliance_reviewer_required");
  }
}
...
function lifecycleStateForEvidence(status: EvidenceStatus): P44EvidenceLifecycleState {
...
export async function createP44EvidenceRequest(
  prisma: PrismaClient,
  input: {
    actorRoleKey: DemoRoleKey;
    auditPersistenceAvailable?: boolean;
    reason: string;
    source: P44EvidenceWorkflowSource;
  ...
  if (!recommendation) {
    throw new P44Phase3ValidationError(["recommendation_scope_required"]);
  }
  ...
  return prisma.$transaction(async (tx) => {
    auditService.assertCriticalAuditWritable({
      ...
      eventType: "p44.evidence.request.created",
    ...
    const item = await tx.evidenceItem.create({
      data: {
        ...
        itemType: "p44_phase3_evidence_request",
...
export async function linkP44EvidenceToObject(
  prisma: PrismaClient,
  input: {
    actorRoleKey: DemoRoleKey;
  ...
  if (!evidence) {
    throw new P44Phase3ValidationError(["evidence_scope_required"]);
  }
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const item = await tx.evidenceItem.create({
      data: {
        evidenceRecordId: evidence.id,
        itemType: "p44_phase3_evidence_link",
...
export async function getP44EvidenceVault(prisma: P44Prisma, input: { actorRoleKey: DemoRoleKey; tenantSlug: DemoTenantSlug }) {
  const session = requireDemoSession({ roleKey: input.actorRoleKey, tenantSlug: input.tenantSlug });
...
export async function projectP44ClientSafeEvidenceSummary(prisma: P44Prisma, input: { evidenceRecordId: string; tenantSlug: DemoTenantSlug }) {
...
export async function rejectP44Evidence(
  prisma: PrismaClient,
  input: {
    actorRoleKey: DemoRoleKey;
    evidenceRecordId: string;
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const item = await tx.evidenceItem.create({
      data: {
        evidenceRecordId: evidence.id,
        itemType: "p44_phase3_evidence_rejection",
    ...
    const audit = await tx.auditEvent.create({
      data: {
        ...
        actorUserId: session.actor.id,
        clientTenantId: session.tenant.id,
        eventType: "p44.evidence.rejected",
        evidenceRecordId: evidence.id,
...
export async function rerequestP44EvidenceAfterRejection(
  prisma: PrismaClient,
  input: {
    actorRoleKey: DemoRoleKey;
  ...
  if (original.status !== EvidenceStatus.RESTRICTED) {
    throw new P44Phase3ValidationError(["rejected_evidence_required"]);
  }
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const item = await tx.evidenceItem.create({
      data: {
        ...
        itemType: "p44_phase3_evidence_re_request",
    ...
    const audit = await tx.auditEvent.create({
      data: {
        ...
        actorUserId: session.actor.id,
        clientTenantId: session.tenant.id,
        eventType: "p44.evidence.re_requested",
        evidenceRecordId: followUp.id,
...
export function certifyP44EvidenceExit(input: {
  lifecycleStates: P44EvidenceLifecycleState[];
