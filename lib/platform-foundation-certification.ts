export type OperationalStage1TicketId =
  | "Operational-1-T01"
  | "Operational-1-T02"
  | "Operational-1-T03"
  | "Operational-1-T04"
...
export type OperationalStage1TicketCertification = {
  acceptance: {
    negative: string;
    positive: string;
  };
  ...
  description: string;
  id: OperationalStage1TicketId;
  proofFiles: readonly string[];
  status: "IMPLEMENTED_WITH_DIRECT_PROOF" | "IMPLEMENTED_WITH_BOUNDARY_PROOF";
};

export const operationalStage1TicketOrder: readonly OperationalStage1TicketId[] = [
  "Operational-1-T01",
  "Operational-1-T02",
  "Operational-1-T03",
  "Operational-1-T04",
...
export const operationalStage1TicketCertifications: readonly OperationalStage1TicketCertification[] = [
  {
    acceptance: {
  ...
  },
  {
    ...
    id: "Operational-1-T11",
    proofFiles: ["tests/providerless-scope.spec.ts", "tests/client-visibility-projection.spec.ts", "tests/client-visibility-payload-sweep.spec.ts"],
    status: "IMPLEMENTED_WITH_DIRECT_PROOF",
  },
  {
    acceptance: {
    ...
    id: "Operational-1-T12",
    proofFiles: ["lib/platform-foundation-certification.ts", "tests/platform-foundation-certification.spec.ts", "docs/00-current/operational-stage1/PHASE1_TICKET_EXTRACTION_AND_EXECUTION_REPORT.md"],
    status: "IMPLEMENTED_WITH_DIRECT_PROOF",
  },
] as const;

export function buildOperationalStage1CertificationSummary() {
  const ticketIds = operationalStage1TicketCertifications.map((ticket) => ticket.id);
  const duplicateTicketIds = ticketIds.filter((ticketId, index) => ticketIds.indexOf(ticketId) !== index);
  const missingTicketIds = operationalStage1TicketOrder.filter((ticketId) => !ticketIds.includes(ticketId));
  const boundaryProofTickets = operationalStage1TicketCertifications
  ...
  return {
    ...
    duplicateTicketIds,
    implementedTicketCount: operationalStage1TicketCertifications.length,
    missingTicketIds,
    ordered: JSON.stringify(ticketIds) === JSON.stringify(operationalStage1TicketOrder),
    status:
