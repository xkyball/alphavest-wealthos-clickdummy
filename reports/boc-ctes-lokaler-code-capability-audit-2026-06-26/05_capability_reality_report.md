...
| Broad demo workflow actions | Remaining call sites only in `j02`, `j03`, `j04`, `j05`, `j09` screen families | `runScreencastDemoAction` and direct demo action calls | `app/api/demo-workflow/route.ts` | Remaining `runJ*` handlers | Persisted compatibility updates across recommendations, evidence, documents, entities and profile/family; export, monitoring, Phase B/C, tenant/governance and platform-admin families are moved or retired | Current-run action-registry proof verifies executable actions are demo-only and moved product families fail closed | `DEMO_COMMAND_BACKED_PARTIAL` |
...
| Tenant-governance typed commands | Yes in Admin/Governance/Export-Ops controls | Yes | Yes | Yes | Yes | Yes | Current-run tenant-governance API proof passed; full browser click journey still pending | `API_BACKED_PARTIAL` |
| Platform-admin typed commands | Yes in Admin controls | Yes | Yes | Audit write | Yes | Yes | Current-run platform-admin API/source proof passed; full browser click journey still pending | `API_BACKED_PARTIAL` |
...
| Demo workflow | Remaining demo-only `actionId` families or blocked moved action IDs | Demo API validation -> action-registry boundary -> remaining `runJ*` handler or fail-closed `410` canonical-route response | Action-specific DB updates only for demo-only families; moved families do not execute through demo workflow | Current action-registry tests prove demo-only vs moved-product split; generic completion claim forbidden | `DEMO_COMMAND_BACKED_PARTIAL` |
...
| Treating `/api/demo-workflow` as product API proof | It still carries demo-only compatibility actions for `j02`, `j03`, `j04`, `j05`, `j09`; moved families now fail closed to canonical APIs | Continue splitting remaining product-like families into typed domain commands instead of stabilizing the demo bus. |
...
| Treating export, tenant governance or platform admin as fully runtime-certified | Their typed APIs and source/API proof passed, but full browser journeys were not run in this audit | Run focused browser/API lifecycle suites before claiming complete delivery. |
...
4. Continue the hard split for the remaining demo-only families: migrate `j04/j05/j09` data-maintenance actions and `j02/j03` advice/evidence actions into typed command clients, then remove those `runScreencastDemoAction` calls from product-like screens.
...
| Demo workflow split plan | Inventory remaining `j02/j03/j04/j05/j09` action IDs and route product-like mutations to typed APIs | High priority; removes ambiguity. |
