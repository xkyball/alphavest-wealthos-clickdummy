...
| Admin tenant setup | Platform / tenant setup | `AdminTenantSetupScreen` | `app/api/admin-tenants/route.ts` `GET`/`POST`, `/api/tenant-governance/actions` `POST` | `p44-phase2-admin-foundation`, demo auth provider service, `tenant-governance-workflow-actions` | Tenant, user, role, policy, team/security-related mutations | Auth/permission/validation responses; no production-auth claim | `API_BACKED_PARTIAL` |
...
| Broad demo workflow actions | Many screens | `runScreencastDemoAction` and direct demo action calls | `app/api/demo-workflow/route.ts` | Remaining `runJ*` handlers | Persisted compatibility updates across recommendations, evidence, documents, entities and profile/family; export, monitoring, Phase B/C and tenant/governance families are moved or retired | Action-specific safety/audit varies; broad current-run proof missing | `DEMO_COMMAND_BACKED_PARTIAL` |
...
| Tenant/admin/governance | Yes by admin API and typed tenant-governance commands | Yes by admin API and `/api/tenant-governance/actions` | Yes by admin snapshot | Policy/security/readmodel | `API_BACKED_PARTIAL` |
