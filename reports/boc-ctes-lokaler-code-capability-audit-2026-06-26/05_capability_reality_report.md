...
The strongest implementation evidence is in document upload, evidence review, client profile/family/entity maintenance, journey commands, export command services, and internal advisor/compliance workflow services. These areas show UI, API, service, DB, guard and test-intent layers in different strengths.

This audit did not run the product runtime, browser flows or focused DB-backed suites. Only `pnpm guard:source` has current-run runtime proof. Therefore this report intentionally marks no product capability as `COMPLETE_VERTICAL_SLICE`. The honest top classification is `STRONG_VERTICAL_CANDIDATE` where the local layer chain is statically strong and tests exist but were not executed in this run.

The largest legacy risk is the broad `/api/demo-workflow` action bus. It contains many real persisted mutations, but it also makes product truth hard to read because action IDs mix demo flow, screencast support, seeded transitions and domain-like workflow state. The bold cleanup path is to split or demote it: typed domain commands become authoritative APIs, and demo-only actions become visibly non-product demo helpers.
...
| Admin tenant setup | Platform / tenant setup | `AdminTenantSetupScreen` | `app/api/admin-tenants/route.ts` `GET`/`POST` | `p44-phase2-admin-foundation`, demo auth provider service | Tenant, user, policy, team/security-related mutations | Auth/permission/validation responses; no production-auth claim | `API_BACKED_PARTIAL` |
...
| Export workflow command spine | Export | Export route family exists; visible UI appears partly demo-action-driven | `app/api/export-workflow/route.ts` | `lib/export-workflow-command-service.ts`, readmodel service | `ExportRequest`, audit events, package metadata | Export safety guards and tests exist | `API_BACKED_PARTIAL` |
...
| Broad demo workflow actions | Many screens | `runScreencastDemoAction` and direct demo action calls | `app/api/demo-workflow/route.ts` | Many `runJ*` handlers | Many persisted updates across recommendations, evidence, documents, entities, export, monitoring | Action-specific safety/audit varies; broad current-run proof missing | `DEMO_COMMAND_BACKED_PARTIAL` |
...
| Export workflow command | Partial/mixed | Yes | Yes | Yes | Yes | Yes | No focused export run | `API_BACKED_PARTIAL` |
...
| Export workflow | Command body, tenant/role, scope/redaction/download/share inputs | API parser -> export command service -> export readmodel | Export request status/package metadata/audit events | Unsafe status, role, redaction, data quality or approval blocks | `API_BACKED_PARTIAL` |
...
| Export requests | Yes via command service/demo | Yes via scoped commands | Yes via readmodel | Package/download readiness | `API_BACKED_COMMAND_SPINE_WITH_UI_GAP_TO_VERIFY` |
| Tenant/admin/governance | Yes by admin/demo actions | Yes by admin/demo actions | Yes by admin snapshot | Policy/security/readmodel | `API_AND_DEMO_COMMAND_BACKED_PARTIAL` |
...
| Treating export as fully wired | Export service/API is strong but inspected UI binding is mixed | Run/export UI proof or rewire visible export actions to typed API. |
| Ignoring schema/test drift | Current static schema count observed 49 models while historical test expectation references 53 | Revalidate and update stale expectations before using schema tests as truth. |

## Bold Legacy-Cleanup Recommendations

1. Promote typed domain command APIs and demote `/api/demo-workflow` to demo-only. Keep it only for screencast seed choreography; move product-like actions into explicit document, export, journey, governance, review and release command routes.
...
4. Make export truth boring and explicit: visible export UI should call `/api/export-workflow`, not demo action IDs, for scope, redaction, approval, generation, download and share.
5. Resolve schema/test drift immediately. Either update stale source-reality expectations to the real 49-model baseline or identify the missing models and restore them deliberately.
...
| Export UI/API directness proof | Verify or rewire export UI to `/api/export-workflow` | High priority before export completion claim. |
| Static affordance purge | Delete/wire/quarantine static action controls | Bold cleanup; do not keep faux controls. |
| Schema alignment refresh | Reconcile 49 model inventory with stale 53-model test expectation | Required before schema alignment is used as acceptance proof. |
...
- This is a static/local report run with one current runtime command: `pnpm guard:source`.
...
- Focused Playwright/API suites were identified but not run.
