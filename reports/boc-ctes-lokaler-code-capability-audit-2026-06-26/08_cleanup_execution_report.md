...
| Typed workflow split | Public `/api/recommendation-review-workflow` now fail-closes all `j08.*` export actions with `LEGACY_EXPORT_DEMO_ACTION_RETIRED` and points to `/api/export-workflow`. |
...
| Proof update | Typed workflow, export approval/download lifecycle, Phase 8, Phase 9 and DBTF tests now assert legacy export actions are retired or use `/api/export-workflow` directly. |
...
| Full app proof suite | Not run; the focused runtime pack above covers export, typed workflow, DBTF/readmodel, document upload, evidence review, AV27 client context and journey API. |
