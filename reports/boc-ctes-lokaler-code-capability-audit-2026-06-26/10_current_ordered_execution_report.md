...
| 2 | `ANALYSIS-1.1.2` Repo-Struktur, Scripts, Tests, Schema und Laufzeitannahmen erfassen | `DONE` | Current inventory verified: 71 registered routes, 32 API route files, 53 Prisma models, 31 enums, 146 test/source files under `tests/**`. |
...
| 4 | `ANALYSIS-2.2` API-, Service- und Workflow-Datenfluss erfassen | `DONE` | Handler/service map refreshed: form APIs, document/evidence APIs, export workflow, journey commands, typed J01/J02/J03/J04/J05/J06/J07/J09/J10/J16/J17 command routes and `/api/recommendation-review-workflow` deleted generic route boundary classified. |
...
| 6 | `ANALYSIS-2.4` Security-, Guard-, Audit- und Test-Beweise erfassen | `DONE` | Guard/test matrix completed. Targeted drift proof pack passed after updating stale API-route truth and first narrowing generic action-id execution; later follow-ups delete the generic route entirely. |
| 7 | `SPEC-1` Report-Taxonomie, Evidence-Regeln und Acceptance Criteria spezifizieren | `DONE` | Report taxonomy, evidence hierarchy, typed-command/deleted-route boundary rules, acceptance criteria and QA labels refreshed from current ANALYSIS findings and current-run proof boundaries. |
| 8 | `IMPL-1.4.1` Capability Matrix und Vertical-Slice Matrix erzeugen | `DONE` | Capability Matrix and Vertical Slice Matrix regenerated with current taxonomy, 32 API-route truth, typed-command statuses and `DEMO_WORKFLOW_ROUTE_DELETED` boundary. |
...
| 11 | `QA-1` Report validieren und Claim-Kontrolle durchführen | `DONE` | QA decision `PASS_WITH_LIMITATIONS`: stale QA claims removed, capability drift gate updated, focused proof pack passed, and current report truth is 53 models / 32 API routes. |
...
| 15 | `FOLLOWUP-2` J01 typed intake/advisor-review boundary | `DONE` | `j01.requestData` moved to `/api/advisor-review/actions`; later `FOLLOWUP-5` deletes the generic workflow route entirely. |
| 16 | `FOLLOWUP-3` Advice/Release-History typed boundary proof for J02/J03 | `DONE` | J03 lifecycle proof migrated from `/api/recommendation-review-workflow` to `/api/advice-release-history/actions`; focused J02/J03 typed-boundary proof pack passed `46/46`. |
...
| 18 | `FOLLOWUP-5` Delete generic workflow route | `DONE` | Deleted the generic route and registry, renamed validation/API tests to recommendation-review, and reconciled generated/source docs to `DEMO_WORKFLOW_ROUTE_DELETED`. |
...
Proceed to `FOLLOWUP-5` after final validation: reconcile remaining historical source-doc `/api/recommendation-review-workflow` claims or split recommendation-review compatibility language further if needed.
