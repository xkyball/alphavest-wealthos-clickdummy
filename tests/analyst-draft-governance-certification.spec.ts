...
import {
  buildOperationalDraftNegativeGateMatrix,
  buildOperationalDraftTraceMap,
  canOperationalDraftAdvanceToAdvisor,
  canOperationalEvidenceBackedDraftMoveToAdvisor,
  certifyOperationalAiDraftGovernanceExit,
  classifyOperationalInternalDraft,
  createOperationalDraftWorkflowClosureNotes,
  createOperationalInternalAiDraft,
  createOperationalStage5ReadinessChecklist,
  getOperationalInternalDraftGovernanceState,
  getOperationalDraftTraceAudit,
  inspectOperationalInternalDraftProjection,
  operationalStage5TicketOrder,
  operationalUnsupportedClaimFeedbackState,
  persistOperationalUnsupportedClaim,
  rebuildOperationalDraftWithEvidence,
  rejectOperationalInternalDraft,
  sweepOperationalAiDraftLeakageSurfaces,
  tagOperationalInternalDraftPayload,
} from "../lib/internal-draft-governance-spine";
import { stableId } from "../lib/stable-id";

const draftKey = "ai-governance-liquidity";
const recommendationId = stableId(`operational:stage5:recommendation:morgan:${draftKey}`);
const evidenceRecordId = stableId(`operational:stage5:evidence:morgan:${draftKey}:accepted`);

test.describe.configure({ mode: "serial" });

test.describe("Operational Stage 5 AI draft governance certification", () => {
  ...
  test.beforeAll(() => {
    ...
    if (!connectionString) {
      throw new Error("DATABASE_URL is required for Operational Stage 5 certification tests.");
    }
  ...
  test("PH5-EXEC creates the ordered readiness checklist without broad parent implementation", () => {
    const blocked = createOperationalStage5ReadinessChecklist({
      analysisComplete: true,
      humanGoNoGo: false,
      predecessorStage4Exit: true,
      specificationComplete: true,
      targetFilesConfirmed: true,
      testsConfirmed: true,
    });
    const ready = createOperationalStage5ReadinessChecklist({
      analysisComplete: true,
      humanGoNoGo: true,
      predecessorStage4Exit: true,
      specificationComplete: true,
    ...
    expect(ready.ticketOrder).toEqual(operationalStage5TicketOrder);
  });

  test("Operational-5-T01 creates internal-only draft state, shells and audit proof", async () => {
    const result = await createOperationalInternalAiDraft(prisma, {
      ...
      reason: "Analyst creates internal AI/rules draft after Stage 4 signal exit.",
      sourceContext: {
        processId: "E-001",
        sourceObjectId: stableId("trigger:morgan:liquidity"),
        sourceObjectType: ObjectType.TRIGGER,
        sourceRefs: ["stage4-signal-exit", "liquidity-workbench"],
      },
      tenantSlug: "morgan",
      title: "Operational internal liquidity draft",
    });
    ...
    expect(audit.eventType).toBe("operational.ai_draft.created_internal");
    expect(audit.result).toBe(AuditResult.PENDING);
  });

  test("Operational-5-T02 tags internal payload fields and blocks client/export projection", () => {
    const tags = tagOperationalInternalDraftPayload({
      clientSummary: "Safe summary",
      clientSummaryDraft: "Internal draft",
      internalRationale: "Internal rationale",
      sourceRefs: ["source-a"],
    });
    const inspection = inspectOperationalInternalDraftProjection({
      clientSummary: "Safe summary",
      clientSummaryDraft: "Internal draft",
      internalRationale: "Internal rationale",
    });
  ...
  test("Operational-5-T03 persists classification and blocks unclassified handoff", async () => {
    const unclassified = await getOperationalInternalDraftGovernanceState(prisma, recommendationId);
    expect(canOperationalDraftAdvanceToAdvisor(unclassified)).toMatchObject({
      allowed: false,
      missing: ["draft_classification", "unsupported_claim_clearance"],
    });

    const result = await classifyOperationalInternalDraft(prisma, {
      actorRoleKey: "analyst",
      classification: {
    ...
    const classified = await getOperationalInternalDraftGovernanceState(prisma, recommendationId);
    ...
    expect(canOperationalDraftAdvanceToAdvisor(classified)).toEqual({
      allowed: true,
      missing: [],
  ...
  test("Operational-5-T04 persists unsupported claims with evidence requirement and internal blocker state", async () => {
    const result = await persistOperationalUnsupportedClaim(prisma, {
      actorRoleKey: "analyst",
  ...
  test("Operational-5-T05 exposes unsupported claim feedback internally and hides it from client", () => {
    const feedback = operationalUnsupportedClaimFeedbackState({
      actorRoleKey: "analyst",
    ...
    const clientFeedback = operationalUnsupportedClaimFeedbackState({
  ...
  test("Operational-5-T06 rejects draft with audit and prevents advisor/release progress", async () => {
    const result = await rejectOperationalInternalDraft(prisma, {
      actorRoleKey: "analyst",
    ...
    const rejectedGate = await getOperationalInternalDraftGovernanceState(prisma, recommendationId);

    expect(result.status).toBe(RecommendationStatus.REVISION_REQUESTED);
    expect(recommendation.clientVisible).toBe(false);
    expect(audit.eventType).toBe("operational.ai_draft.rejected");
  ...
  test("Operational-5-T07 rebuilds only with accepted scoped evidence", async () => {
    await expect(
      rebuildOperationalDraftWithEvidence(prisma, {
        actorRoleKey: "analyst",
        evidenceIds: [],
    ...
    await prisma.evidenceRecord.upsert({
      create: {
        ...
        title: "Operational accepted draft evidence",
    ...
    const result = await rebuildOperationalDraftWithEvidence(prisma, {
      actorRoleKey: "analyst",
  ...
  test("Operational-5-T08 maps internal evidence trace while client projection exposes no rationale", async () => {
    const trace = await buildOperationalDraftTraceMap(prisma, {
      actorRoleKey: "analyst",
      recommendationId,
      tenantSlug: "morgan",
    });
  ...
  test("Operational-5-T09 marks missing audit as a completion blocker and passes complete trace audit", async () => {
    const missingAudit = await getOperationalDraftTraceAudit(prisma, {
      recommendationId,
      requiredEvents: ["operational.ai_draft.nonexistent"],
    });
    const traceAudit = await getOperationalDraftTraceAudit(prisma, { recommendationId });

    expect(missingAudit.blocksCompletionClaim).toBe(true);
    expect(missingAudit.missing).toEqual(["operational.ai_draft.nonexistent"]);
    expect(traceAudit.blocksCompletionClaim).toBe(false);
    expect(traceAudit.auditRows).toBeGreaterThanOrEqual(3);
  });

  test("Operational-5-T10 sweeps API, client-route and export payloads for internal leakage", () => {
    const sweep = sweepOperationalAiDraftLeakageSurfaces([
      {
  ...
  test("Operational-5-T11 allows advisor handoff only after clear classification and evidence-backed rebuild", async () => {
    await classifyOperationalInternalDraft(prisma, {
      actorRoleKey: "analyst",
      classification: {
        classification: "advice_relevant",
        riskLevel: "high",
    ...
    const recommendation = await prisma.recommendation.findUniqueOrThrow({ where: { id: recommendationId } });
    const gateState = await getOperationalInternalDraftGovernanceState(prisma, recommendationId);
    const gate = canOperationalEvidenceBackedDraftMoveToAdvisor({
      ...gateState,
      status: recommendation.status,
    });
    ...
    expect(gate).toEqual({
      allowed: true,
      missing: [],
      ticketId: "Operational-5-T11-EXEC",
    });
  });

  test("Operational-5-T12 creates negative gate matrix for unsupported, unclassified, rejected and leakage cases", () => {
    const matrix = buildOperationalDraftNegativeGateMatrix({
      leakageSweep: sweepOperationalAiDraftLeakageSurfaces([
        {
      ...
      unsupportedClaimFeedback: operationalUnsupportedClaimFeedbackState({
        actorRoleKey: "analyst",
  ...
  test("Operational-5-T13 creates closure notes without demo-only completion claim", () => {
    const notes = createOperationalDraftWorkflowClosureNotes({
      ...
      testFiles: ["tests/analyst-draft-governance-certification.spec.ts"],
    });
  ...
  test("Operational-5-T14 certifies Stage 5 exit only with all tickets and no hidden blockers", () => {
    const negativeGateMatrix = buildOperationalDraftNegativeGateMatrix({
      leakageSweep: sweepOperationalAiDraftLeakageSurfaces([
        {
          name: "unsafe client payload",
          payload: { aiDraft: "Internal AI draft" },
      ...
      rejectedDraftGate: { allowed: false, missing: ["draft_not_rejected"], ticketId: "Operational-5-T11-EXEC" },
      unclassifiedDraftGate: { allowed: false, missing: ["draft_classification"], ticketId: "Operational-5-T03-EXEC" },
      unsupportedClaimFeedback: operationalUnsupportedClaimFeedbackState({
        actorRoleKey: "analyst",
        evidenceRequirement: "Reviewed evidence required.",
        reason: "Unsupported claim.",
    ...
    const closureNotes = createOperationalDraftWorkflowClosureNotes({
      proofNotes: ["All Operational Stage 5 E-domain proof layers are mapped."],
      routeFiles: ["app/api/recommendation-review-workflow/route.ts"],
      serviceFiles: ["lib/internal-draft-governance-spine.ts"],
      testFiles: ["tests/analyst-draft-governance-certification.spec.ts"],
    });
    const certification = certifyOperationalAiDraftGovernanceExit({
      closureNotes,
      completedTickets: [...operationalStage5TicketOrder],
      negativeGateMatrix,
      payloadBlockers: [],
    });

    expect(certification.certified).toBe(true);
    expect(certification.status).toBe("Operational_PHASE5_AI_GOVERNANCE_READY");
