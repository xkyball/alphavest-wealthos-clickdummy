...
test.describe("AV27 Phase 2 client context closure", () => {
  ...
  test("P2-T03 creates tenant-linked entities and invalid submissions do not create partial rows", async ({ request }) => {
    ...
    const invalidName = `AV27 Invalid ${Date.now()}`;
    ...
    const entityName = `AV27 Summit Holdings ${Date.now()}`;
    const createResponse = await request.post("/api/entities", {
      data: {
        ...
        name: entityName,
        ownerSummary: "AV27 tenant-linked entity proof.",
        registrationNumber: "AV27-2026",
        riskRating: "Medium",
        roleKey: "family_cfo",
        tenantSlug: "summit",
      },
  ...
  test("P2-T04 derives sensitivity visibility and hides internal context without inference", async ({ request }) => {
    ...
    const hiddenName = `AV27 Hidden ${Date.now()}`;
    const hiddenEntity = `AV27 Hidden Entity ${Date.now()}`;
