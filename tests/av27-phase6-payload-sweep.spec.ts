...
function decisionPayload(overrides: Partial<DecisionVisibilityPayload> = {}): DecisionVisibilityPayload {
  ...
  return {
    ...
    id: "decision:bennett:av27-phase6",
    internalRationale: "Internal rationale.",
    releasedAt: null,
    sensitivity: "RESTRICTED",
    submittedAt: "2026-06-25T08:00:00.000Z",
    title: "AV27 phase 6 decision",
    visibilityStatus: "COMPLIANCE_VISIBLE",
    ...overrides,
  };
}

test.describe("AV27 Phase 6 client visibility and export closure", () => {
  test("AV27-P6-T01 keeps AI/rules draft internal-only across client and export payloads", () => {
  ...
  test("AV27-P6-T02 projects only released client-safe recommendation and decision payloads", () => {
    const principal = createDemoSession({ roleKey: "principal", tenantSlug: "bennett" });
  ...
  test("AV27-P6-T03 treats client-safe summary as release-derived and blocks draft summary fields", () => {
    const safeSummary = inspectAv27Phase6ClientPayload({ clientSummary: "Released client-safe summary." }, { surface: "api" });
    const unsafeSummary = inspectAv27Phase6ClientPayload({
      clientSummary: "Released client-safe summary.",
  ...
  test("AV27-P6-T04 excludes forbidden internal payloads from export surfaces", () => {
  ...
  test("AV27-P6-T05 sweeps API, UI and export payloads with one ordered ticket matrix", () => {
    ...
    expect(av27Phase6TicketOrder).toEqual(["AV27-P6-T01", "AV27-P6-T02", "AV27-P6-T03", "AV27-P6-T04", "AV27-P6-T05"]);
    expect(av27Phase6AllowedClientPayloadFields).toEqual(expect.arrayContaining(["clientSummary", "decisionState", "title"]));
    expect(av27Phase6AllowedExportPayloadFields).toEqual(expect.arrayContaining(["clientSummary", "releasedAt"]));
    expect(av27Phase6ForbiddenPayloadFields).toEqual(expect.arrayContaining([
      "aiDraft",
      "analystNotes",
      "clientSummaryDraft",
      "complianceNotes",
