...
import {
  certifyOperationalEvidenceExit,
  createOperationalEvidenceRequest,
  evidenceRequestFeedbackState,
  getOperationalEvidenceVault,
  linkOperationalEvidenceToObject,
  projectOperationalClientSafeEvidenceSummary,
  rejectOperationalEvidence,
  rerequestOperationalEvidenceAfterRejection,
} from "../lib/evidence-lifecycle-service";
...
test.describe("Operational Stage 3 evidence lifecycle certification", () => {
  ...
  test.beforeAll(() => {
    ...
    if (!connectionString) {
      throw new Error("DATABASE_URL is required for Operational Stage 3 certification tests.");
    }
  ...
  test("Operational-3-T01 creates scoped evidence request state, assignee, reason and audit without sufficiency or release", async () => {
    const result = await createOperationalEvidenceRequest(prisma, {
      actorRoleKey: "compliance_officer",
      reason: "Operational Stage 3 needs refreshed liquidity evidence before release.",
      source: "compliance",
    ...
    expect(audit.eventType).toBe("operational.evidence.request.created");
    expect(audit.result).toBe(AuditResult.PENDING);
  });

  test("Operational-3-T02 exposes loading, validation failed, blocked, success and retry feedback states without advancing workflow", () => {
  ...
  test("Operational-3-T03 links evidence to the scoped object without claiming gate support", async () => {
    const result = await linkOperationalEvidenceToObject(prisma, {
  ...
  test("Operational-3-T04 consumes linked evidence as blocker context, not release evidence", async () => {
    const vault = await getOperationalEvidenceVault(prisma, { actorRoleKey: "compliance_officer", tenantSlug: "morgan" });
  ...
  test("Operational-3-T05 projects client-safe evidence summary only after release", async () => {
    const blocked = await projectOperationalClientSafeEvidenceSummary(prisma, {
  ...
  test("Operational-3-T06 keeps unreleased summary payloads fail-closed and free of internal fields", async () => {
    const blocked = await projectOperationalClientSafeEvidenceSummary(prisma, {
      evidenceRecordId: linkedEvidenceRecordId,
      tenantSlug: "morgan",
    });
  ...
  test("Operational-3-T07 returns scoped evidence vault list/detail lifecycle state", async () => {
    const vault = await getOperationalEvidenceVault(prisma, { actorRoleKey: "analyst", tenantSlug: "morgan" });
  ...
  test("Operational-3-T08 maps vault lifecycle states without unlocking rejected or stale evidence", async () => {
    const vault = await getOperationalEvidenceVault(prisma, { actorRoleKey: "compliance_officer", tenantSlug: "morgan" });
  ...
  test("Operational-3-T09 rejects evidence with reason, reviewer, status and audit", async () => {
    const result = await rejectOperationalEvidence(prisma, {
      actorRoleKey: "compliance_officer",
      evidenceRecordId: linkedEvidenceRecordId,
    ...
    expect(audit.eventType).toBe("operational.evidence.rejected");
    expect(audit.result).toBe(AuditResult.BLOCKED);
  });

  test("Operational-3-T10 creates a follow-up request after rejection and preserves rejection history", async () => {
    const result = await rerequestOperationalEvidenceAfterRejection(prisma, {
      actorRoleKey: "compliance_officer",
    ...
    const [original, followUp, rejectionItems] = await Promise.all([
      ...
      prisma.evidenceItem.findMany({
        where: { evidenceRecordId: rejectedEvidenceRecordId, itemType: "operational_stage3_evidence_rejection" },
      }),
    ]);
  ...
  test("Operational-3-T11 proves upload-only, unlinked, rejected and stale evidence never unlock gates", async () => {
    const rejected = await projectOperationalClientSafeEvidenceSummary(prisma, {
      evidenceRecordId: rejectedEvidenceRecordId,
      tenantSlug: "morgan",
    });
    const followUp = await projectOperationalClientSafeEvidenceSummary(prisma, {
      evidenceRecordId: followUpEvidenceRecordId,
      tenantSlug: "morgan",
    });

    await expect(
      linkOperationalEvidenceToObject(prisma, {
    ...
    ).rejects.toThrow("Operational Stage 3 evidence lifecycle action is not permitted.");

    expect(rejected.allowed).toBe(false);
    expect(followUp.allowed).toBe(false);
  });

  test("Operational-3-T12 certifies Stage 3 exit with positive and negative proof coverage", () => {
    const certification = certifyOperationalEvidenceExit({
      ...
      ticketIds: [
        "Operational-3-T01-IMPL",
        "Operational-3-T02-IMPL",
        "Operational-3-T03-IMPL",
        "Operational-3-T04-IMPL",
        "Operational-3-T05-IMPL",
        "Operational-3-T06-IMPL",
        "Operational-3-T07-IMPL",
        "Operational-3-T08-IMPL",
        "Operational-3-T09-IMPL",
        "Operational-3-T10-IMPL",
        "Operational-3-T11-IMPL",
        "Operational-3-T12-IMPL",
      ],
    });
