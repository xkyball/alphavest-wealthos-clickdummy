  ...
  for (const route of operationalVisualRoutes) {
    test(`${route.name} passes audited 1400x900 screenshot contract`, async ({ page }) => {
      ...
      const audit = await page.evaluate(() => {
        ...
        return {
          badgeClusterCount,
          forbiddenOperationalText: /Process Runtime|Search scoped|Scoped context|Export Scope Selection|Select export scope|scope selection|Scope\s+Scope|Export Redaction|Payload Redaction Operations|\bpayload\b|Stage:|Redactions:|Approval unavailable|Approval blocked until preview|Approval blocked until|Blocked before preview|Forbidden payload|blocked until .*gates? pass|release gates? pass|visibility gates? pass|Dense operations safety gate|Workflow Badges|workflow proof|proof drawer|command spine|drawer handoff|Access request gate|Export redaction gate|Selected\s+Queue|Scoped\s+Detail|Gated\s+Step|policy-checked request|Review policy-checked|Download confirmation|Download review|confirmation box|Advisor approval is not release|Compliance pending|Client blocked|Draft contained|Release not set|Advisor candidate only|Not released/i.test(visibleText),
