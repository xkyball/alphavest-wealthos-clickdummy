...
import {
  buildP44AdvisorReturnNegativeMatrix,
  certifyP44AdvisorReviewExit,
  createP44AdvisorEvidenceRequestFeedbackState,
  createP44AdvisorQueueTriage,
  createP44OptionComparison,
  createP44Phase6ReadinessChecklist,
  evaluateP44AdvisorQueueScope,
  evaluateP44OptionDecisionGuard,
  p44Phase6TicketOrder,
  recordP44AdvisorApprovalWithoutRelease,
  requestP44AdvisorMoreEvidence,
  returnP44AdvisorReviewToAnalyst,
  sweepP44AdvisorNotRelease,
} from "../lib/p44-phase6-advisor-review-closure";
import { stableId } from "../lib/stable-id";

const draftKey = "advisor-review-liquidity";
const recommendationId = stableId(`p44:phase6:recommendation:morgan:${draftKey}`);

test.describe.configure({ mode: "serial" });

test.describe("P44 Phase 6 advisor review closure certification", () => {
  ...
  test.beforeAll(() => {
    ...
    if (!connectionString) {
      throw new Error("DATABASE_URL is required for P44 Phase 6 certification tests.");
    }
  ...
  test("PH6-EXEC creates ordered readiness after analysis/spec and Phase 5 exit", () => {
    const blocked = createP44Phase6ReadinessChecklist({
      analysisComplete: true,
      predecessorPhase5Exit: false,
      specificationComplete: true,
      targetFilesConfirmed: true,
      testsConfirmed: true,
    });
    const ready = createP44Phase6ReadinessChecklist({
      analysisComplete: true,
      predecessorPhase5Exit: true,
      specificationComplete: true,
      targetFilesConfirmed: true,
      testsConfirmed: true,
    });
    ...
    expect(blocked.missing).toEqual(["p44_phase5_exit"]);
    expect(ready.ready).toBe(true);
    expect(ready.ticketOrder).toEqual(p44Phase6TicketOrder);
  });

  test("P44-6-T01 persists advisor queue triage/readiness and does not release", async () => {
    const result = await createP44AdvisorQueueTriage(prisma, {
      ...
      reason: "Phase 5 AI governance exit produced an advisor-review candidate.",
      sourceRefs: ["PH5-QA", "P44-5-T14"],
      tenantSlug: "morgan",
      title: "P44 Phase 6 advisor review candidate",
    });
    ...
    expect(audit.eventType).toBe("p44.advisor_queue.triaged");
    expect(audit.result).toBe(AuditResult.PENDING);
  });

  test("P44-6-T02 allows scoped advisor queue access and denies wrong object scope", async () => {
    const scoped = await evaluateP44AdvisorQueueScope(prisma, {
      actorRoleKey: "senior_wealth_advisor",
      allowedObjectIds: [recommendationId],
      queueItemId,
      tenantSlug: "morgan",
    });
    const wrongObject = await evaluateP44AdvisorQueueScope(prisma, {
      actorRoleKey: "senior_wealth_advisor",
      allowedObjectIds: [stableId("p44:phase6:not-the-recommendation")],
      queueItemId,
      tenantSlug: "morgan",
    });
  ...
  test("P44-6-T03 persists option comparison rows and scoped evidence with client-safe projection", async () => {
    const result = await createP44OptionComparison(prisma, {
      actorRoleKey: "senior_wealth_advisor",
      options: [
        {
  ...
  test("P44-6-T04 lets advisor proceed only with scoped accepted evidence", async () => {
    const accepted = await evaluateP44OptionDecisionGuard(prisma, {
      actorRoleKey: "senior_wealth_advisor",
    ...
    const missingEvidence = await evaluateP44OptionDecisionGuard(prisma, {
      actorRoleKey: "senior_wealth_advisor",
      evidenceIds: [],
      optionId: selectedOptionId,
      recommendationId,
    ...
    const wrongRole = await evaluateP44OptionDecisionGuard(prisma, {
      actorRoleKey: "analyst",
      evidenceIds: [selectedEvidenceId],
      optionId: selectedOptionId,
      recommendationId,
  ...
  test("P44-6-T05 records advisor evidence request with audit and no release", async () => {
    const result = await requestP44AdvisorMoreEvidence(prisma, {
      actorRoleKey: "senior_wealth_advisor",
    ...
    expect(audit.eventType).toBe("p44.advisor.requested_evidence");
  });

  test("P44-6-T06 exposes request feedback states and failed state cannot advance approval", () => {
    const states = ["loading", "validation", "success", "error", "blocked", "retry"] as const;
    const feedback = states.map((lifecycle) => createP44AdvisorEvidenceRequestFeedbackState({ lifecycle }));
  ...
  test("P44-6-T07 returns advisor review to analyst with reason, target role and audit", async () => {
    const result = await returnP44AdvisorReviewToAnalyst(prisma, {
      actorRoleKey: "senior_wealth_advisor",
    ...
    returnAuditCreated = audit.eventType === "p44.advisor.returned_to_analyst";
  ...
  test("P44-6-T08 proves reject/return blocks release and client/export projection", () => {
    const matrix = buildP44AdvisorReturnNegativeMatrix({
  ...
  test("P44-6-T09 proves advisor approval, evidence request and reject do not release", async () => {
    const approval = await recordP44AdvisorApprovalWithoutRelease(prisma, {
      actorRoleKey: "senior_wealth_advisor",
    ...
    const sweep = sweepP44AdvisorNotRelease({
  ...
  test("P44-6-T10 certifies Phase 6 exit only with all tickets and no advisor bypass", () => {
    const returnNegativeMatrix = buildP44AdvisorReturnNegativeMatrix({
      clientProjectionVisible: returnSnapshot.clientVisible,
      complianceReleased: returnSnapshot.complianceStatus === ComplianceStatus.RELEASED,
      returnAuditCreated,
      returnStatus: returnSnapshot.recommendationStatus,
    });
    const notReleaseSweep = sweepP44AdvisorNotRelease({
      advisorApproval: advisorApprovalSnapshot,
      evidenceRequest: evidenceRequestSnapshot,
      returnToAnalyst: returnSnapshot,
    });
    const certification = certifyP44AdvisorReviewExit({
      completedTickets: [...p44Phase6TicketOrder],
      notReleaseSweep,
      returnNegativeMatrix,
    });

    expect(certification.certified).toBe(true);
    expect(certification.status).toBe("P44_PHASE6_ADVISOR_REVIEW_READY");
