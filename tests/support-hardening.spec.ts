...
function safeScopeItem(label: string) {
  return {
    access: "Allowed",
    id: stableId(`stage9-export-workflow:${label}`),
...
async function prepareApprovalRequiredExport(request: APIRequestContext) {
  const scope = await exportCommand(request, {
    command: "SET_SCOPE",
    reason: "Select client-safe released objects for stage 9 blocker proof.",
...
async function createSummitHighSeverityIssue(
  ...
  await prisma.dataQualityIssue.upsert({
    where: { id: stableId(`data-quality:summit:stage9-release-blocker:${targetId}`) },
    create: {
      id: stableId(`data-quality:summit:stage9-release-blocker:${targetId}`),
      clientTenantId: summitTenantId,
      description: "Stage 9 release/export blocker: conflicting beneficial ownership data needs review.",
...
test.describe.serial("Stage 9 support hardening", () => {
  let prisma: PrismaClient | undefined;

  test.beforeEach(() => {
    const connectionString = process.env.DATABASE_URL;
    if (!connectionString) {
      throw new Error("DATABASE_URL is required for Stage 9 support hardening tests.");
    }
