...
test.describe("WORKFLOW02 worksurface shell", () => {
  test("centralizes the shared worksurface renderer and route adoption", () => {
    ...
    expect(shell).toContain('data-testid="workflow02-worksurface-shell"');
  ...
  for (const route of worksurfaceRoutes) {
    test(`${route.pageId} uses the WORKFLOW02 worksurface shell`, async ({ page }) => {
      ...
      const shell = page.getByTestId("workflow02-worksurface-shell");
      await expect(shell).toHaveCount(1);
      await expect(shell).toHaveAttribute("data-workflow02-route-id", route.pageId);
      await expect(shell).toHaveAttribute("data-workflow02-worksurface", route.worksurface);
